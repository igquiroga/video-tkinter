...
def run():
	...
	def stopVideo():
		videoplayer.stop()
	playButton = Button(frameButtons,text="Pause",bg=bgSecondary,fg=fgWhite,command=playVideo)
	playButton.pack(side=LEFT,fill=X,expand=1)

	stopButton = Button(frameButtons,text="Stop",bg=bgSecondary,fg=fgWhite,command=stopVideo)
	...
	videoplayer.load(r"igquiroga.mp4")
	...
	labelTime.pack() 
	def timeVideo():
		labelTime["text"] = "Time: "+str(math.floor(videoplayer.current_duration()))+"s"
